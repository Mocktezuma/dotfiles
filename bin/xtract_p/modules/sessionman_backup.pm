...
sub LogUDPsession {
   (my $Sessions, 
    ...
    $Sessions->{$UDP}->{$dst_port}->{$src_port}->{$dst_ip}->{$src_ip} .= $$payload;
...
sub LogTCPsession {      # simply log the src/dst port+IP information together with the sequence number of the datagram
    ...
    if (! $Sessions->{$TCP}->{$dst_port}->{$src_port}->{$dst_ip}->{$src_ip} ) {    #  beginning of new session

	push my @stream_log,$connection;                                  
	$Sessions->{$TCP}->{$dst_port}->{$src_port}->{$dst_ip}->{$src_ip} = \@stream_log; 

    } 

    else {

	push @{$Sessions->{$TCP}->{$dst_port}->{$src_port}->{$dst_ip}->{$src_ip}},$connection;
...
sub Clean_TCP_Sessions {            # eliminate duped packets, write final payloads for streams
	...
	my $dst_port = $_;       
	my $SRC_PORTS_HASHref = $DST_PORTS_HASHref->{$dst_port};

	for (keys %$SRC_PORTS_HASHref) {

	    my $src_port = $_;
	    my $DST_IP_HASHref = $SRC_PORTS_HASHref->{$src_port};
	    
	    for (keys %$DST_IP_HASHref) {
		    
		my $dst_ip = $_;
		my $SRC_IP_HASHref = $DST_IP_HASHref->{$dst_ip};
    ...
    foreach my $transport_layer (@$protocols) {
	   ...
	   for (keys %$SRC_PORTS_HASHref) {                          
		   ...
		   for (keys %$SRC_IP_HASHref) {
		       ...
		       my $key_string = sprintf($TEMPLATE,$src_ip,$dst_ip);

		       my $IdentifyingPort = $FLAG{nomin} ? $src_port : Min($src_port,$dst_port);
			   
		       my $type = ReturnTypeName($transport_layer,$IdentifyingPort,$ProtocolNames,$TransportNames);
