...
connection.onInitialize((params) => {
    ...
    const tagPaths = params.initializationOptions.tagPaths;
    const attributePaths = params.initializationOptions.attributePaths;
    const htmlTags = {};
        ...
        tagPaths.forEach(path => {
            try {
                if (fs.existsSync(path)) {
                    const tagSet = tagDefinitions_1.parseTagSet(fs.readFileSync(path, 'utf-8'));
    ...
    if (attributePaths) {
        attributePaths.forEach(path => {
            try {
                if (fs.existsSync(path)) {
                    const attributeSet = tagDefinitions_1.parseAttributes(fs.readFileSync(path, 'utf-8'));
    ...
    languageModes = languageModes_1.getLanguageModes(initializationOptions ? initializationOptions.embeddedLanguages : { css: true, javascript: true }, workspace, htmlTags, htmlAttributes);
    documents.onDidClose(e => {
